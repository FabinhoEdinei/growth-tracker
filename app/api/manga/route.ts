...
// Lista capítulos e páginas da pasta public/manga/
// Estrutura esperada:
//   public/manga/cap-01/001.jpg
//   public/manga/cap-01/002.jpg
//   public/manga/cap-02/001.jpg ...
...
export interface MangaPage    { index: number; src: string; }
...
export async function GET(request: Request) {
  ...
  const capId = searchParams.get('cap'); // se passado, retorna só esse cap

  const mangaRoot = path.join(process.cwd(), 'public', 'manga');

  // Se a pasta não existe, retorna estrutura vazia mas não quebra
  if (!fs.existsSync(mangaRoot)) {
    return NextResponse.json({ capitulos: [], total: 0, aviso: 'Pasta public/manga/ não encontrada' });
  }

  try {
    const dirs = fs.readdirSync(mangaRoot)
      .filter(d => {
        const full = path.join(mangaRoot, d);
    ...
    const capitulos: MangaCapitulo[] = dirs.map((dir, i) => {
      ...
      // Tenta ler info.json para título customizado
    ...
    return NextResponse.json({ capitulos, total: capitulos.length });
