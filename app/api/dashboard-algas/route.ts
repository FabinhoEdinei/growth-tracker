...
function calcularPostsPorMes(posts: PostMeta[]) {
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    const key = `${d.getFullYear()}-${d.getMonth()}`;
    return { month: labels[d.getMonth()], posts: meses[key] || 0, year: d.getFullYear() };
  });
}
...
function calcularCategorias(posts: PostMeta[]) {
  ...
  return Object.entries(map)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map(([name, count], i) => ({ name, count, color: cores[i] ?? '#00ff88' }));
}
...
export async function GET() {
  try {
    ...
    // Mês anterior para calcular crescimento
    ...
    return NextResponse.json({
      blog: {
        ...
        recentes:       recentes.filter(p => todosBlogs.find(b => b.slug === p.slug)),
      },
      jornal: {
