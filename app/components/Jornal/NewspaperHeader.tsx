...
interface NewspaperHeaderProps {
  // Props opcionais — quando passadas, exibe a barra de navegação
  paginaAtual?:  number;
  totalPaginas?: number;
  totalPosts?:   number;
}
...
export const NewspaperHeader = ({
  ...
  return (
    <header className="newspaper-header">
      ...
      {/* ── Navegação de paginação — no lugar do "GRATUITO" ── */}
      {mostraNave ? (
        <div className="nav-bar">
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:3, flex:1 }}>
            <span style={{ fontFamily:"'Courier New',monospace", fontSize:7, color:'rgba(200,168,75,0.55)', letterSpacing:1.5 }}>
        ...
        /* Sem paginação — mostra GRATUITO original */
        <div className="nav-bar nav-bar--gratuito">
          <Link href="/" style={btnHome}>⌂ HOME</Link>
          <span className="price">GRATUITO</span>
          <span style={{ opacity:0, pointerEvents:'none', ...btnHome }}>—</span>
        </div>
      )}
      ...
      <style jsx>{`
        .newspaper-header {
          text-align: center;
          padding: 12px 30px;
          background: linear-gradient(180deg, #2a1810 0%, #1f120d 50%, #1a0f0a 100%);
          border: 2px solid #8B7355;
          position: relative;
          margin: 0 10px 0;
          box-shadow: 0 4px 20px rgba(0,0,0,0.5), inset 0 1px 0 rgba(218,165,32,0.15);
          overflow: visible;
        }
        ...
        .corner-ornament.top-left    { top:0; left:0;   filter:drop-shadow(0 0 6px rgba(218,165,32,0.8)); }
        .corner-ornament.top-right   { top:0; right:0;  filter:drop-shadow(0 0 6px rgba(218,165,32,0.8)); }
        .corner-ornament.bottom-left { bottom:0; left:0;  filter:drop-shadow(0 0 6px rgba(218,165,32,0.8)); }
        .corner-ornament.bottom-right{ bottom:0; right:0; filter:drop-shadow(0 0 6px rgba(218,165,32,0.8)); }
        ...
        @media (max-width: 768px) {
          .newspaper-header { padding:10px 18px; margin:0 5px 0; }
          .corner-ornament  { width:60px; height:60px; }
          .newspaper-title  { font-size:18px; letter-spacing:2px; margin:6px 0; }
          .newspaper-meta   { font-size:7px; margin-top:8px; gap:3px; }
          .bottom-ornament  { font-size:8px; letter-spacing:6px; margin-top:8px; }
          .nav-bar          { gap:5px; }
        }

        @media (max-width: 480px) {
          .newspaper-header { padding:8px 12px; }
          .corner-ornament  { width:48px; height:48px; }
          .newspaper-title  { font-size:15px; letter-spacing:1.5px; }
          .bottom-ornament  { font-size:7px; letter-spacing:5px; }
        }
      `}</style>
    </header>
  );
